...
CFLAGS?=-ffunction-sections -Os -fdata-sections -Wall -fvisibility=hidden -IVrApi/Include
LDFLAGS?=-Wl,--gc-sections -s VrApi/Libs/Android/arm64-v8a/Release/libvrapi.so
...
makecapk.apk : $(TARGETS) $(EXTRA_ASSETS_TRIGGER) AndroidManifest.xml
	...
	cp VrApi/Libs/Android/arm64-v8a/Release/libvrapi.so makecapk/lib/arm64-v8a/
	#$(NDK)/toolchains/llvm/prebuilt/$(OS_NAME)/bin/aarch64-linux-android-strip  makecapk/lib/arm64-v8a/libvrapi.so
	rm -rf temp.apk
